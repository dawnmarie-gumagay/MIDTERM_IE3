...
st.markdown(
    ...
    .reportview-container {
        background-color: #E0F7FA;  /* Background Color: Light Blue */
        color: #2F4F4F;  /* Text Color: Dark Slate Gray */
    }
    .st-button {
        background-color: #001f3f;  /* Primary Color: Navy Blue */
        color: #FFFFFF;
    }
    .stButton>button {
        color: #FFFFFF;
...
# Load the dataset
file_path = r"C:\Users\dawn\OneDrive - Cebu Institute of Technology University\4TH YEAR_1ST SEM\IE3_CSIT342\MIDTERM\titanic_dataset.csv"
...
# Dropdown for choosing visualizations
...
if visualization_type == "Survival Rates by Class":
    ...
    sns.barplot(x=class_survival_rate.index, y=class_survival_rate.values, ax=ax1, color='#001f3f')  # Navy Blue
    ax1.set_title("Survival Rate by Class", color='#001f3f')  # Title color
    ax1.set_ylabel("Survival Rate", color='#001f3f')  # Y-axis label color
    ax1.set_xlabel("Passenger Class", color='#001f3f')  # X-axis label color
    st.pyplot(fig1)
    ...
    sns.barplot(x=gender_survival_rate.index, y=gender_survival_rate.values, ax=ax2, color='#001f3f')  # Navy Blue
    ax2.set_title("Survival Rate by Gender", color='#001f3f')  # Title color
    ax2.set_ylabel("Survival Rate", color='#001f3f')  # Y-axis label color
    ax2.set_xlabel("Gender", color='#001f3f')  # X-axis label color
    st.pyplot(fig2)
    ...
    fig3 = px.pie(filtered_data, names='Pclass', title="Percentage of Passengers by Class", hole=0.3, 
                   color_discrete_sequence=['#001f3f', '#FF6F61', '#FF6F61'])  # Navy Blue and Coral
    ...
    fig4 = px.pie(filtered_data, names='Survived', title="Survival Rate Percentage", hole=0.3, 
                   color_discrete_sequence=['#001f3f', '#FF6F61'])  # Navy Blue and Coral
    ...
    sns.heatmap(corr_matrix, annot=True, cmap="Blues", ax=ax5)  # Change color map for heatmap
    ax5.set_title("Correlation Heatmap of Features", color='#001f3f')  # Title color
    st.pyplot(fig5)
    ...
    sns.histplot(filtered_data['Age'], bins=20, kde=True, ax=ax6, color='#001f3f')  # Navy Blue
    ax6.set_title("Age Distribution", color='#001f3f')  # Title color
    ax6.set_xlabel("Age", color='#001f3f')  # X-axis label color
    st.pyplot(fig6)
    ...
    sns.histplot(filtered_data['Fare'], bins=20, kde=True, ax=ax7, color='#001f3f')  # Navy Blue
    ax7.set_title("Fare Distribution", color='#001f3f')  # Title color
    ax7.set_xlabel("Fare", color='#001f3f')  # X-axis label color
    st.pyplot(fig7)
